...
check_simdata_groups <- function(dataFile,
                                 mu0, Sigma0,
                                 mu1, Sigma1,
                                 r = NULL) {
  ...
  tryCatch({
    ...
    Z_clean <- Z[data$labelsVrais == 0, , drop = FALSE]
    Z_cont  <- Z[data$labelsVrais == 1, , drop = FALSE]
    
    n <- nrow(Z)
    
    # -----------------------------
    # 1. check number of outliers
    # -----------------------------
    ok_nb_outliers <- TRUE
    if (!is.null(r)) {
      ok_nb_outliers <- sum(data$labelsVrais == 1) == r * n / 100
    }
    
    
    # Mahalanobis clean
    invS0 <- solve(cov(Z_clean))
    d2_clean <- apply(Z_clean, 1, function(x)
      t(x - colMeans(Z_clean)) %*% invS0 %*% (x - colMeans(Z_clean))
    )
    ok_clean_maha_clean <- ks.test(d2_clean, "pchisq", df = ncol(Z_clean))$p.value > 0.05
    
    # -----------------------------
    # 3. CONTAMINATED empirical checks
    # -----------------------------
    if(r != 0){
    mean_cont <- colMeans(Z_cont)
    cov_cont  <- cov(Z_cont)
    
    # Mahalanobis cont
    invS1 <- solve(cov(Z_cont))
    d2_cont <- apply(Z_cont, 1, function(x)
      t(x - colMeans(Z_cont)) %*% invS1 %*% (x - colMeans(Z_cont))
    )
    ok_cont_maha_cont <- ks.test(d2_cont, "pchisq", df = ncol(Z_cont))$p.value > 0.05
    }
    ...
    message("✖ ERROR : ", dataFile, " -> ", e$message)
    FALSE
  })
}
check_fit <- function(fitFile, variance, outliers_labels, distances) {
  ...
  tryCatch({
    ...
    ok_var  <- err_var < 1e-3
    ok_dist <- err_dist < 1e-3
    ok_out  <- (n_mismatch_out == 0)
    
    ok <- ok_var && ok_dist && ok_out
    
    if (ok) {
      message("✔ OK : ", fitFile)
    } else {
      message("✖ FAILED : ", fitFile)
      
      if (!ok_var)
        message("   Variance failed (Frobenius = ", signif(err_var, 6), ")")
      
      if (!ok_dist)
        message("   Distances failed (max diff = ", signif(err_dist, 6), ")")
      
      if (!ok_out)
        message("   Outliers failed (", n_mismatch_out, " mismatches)")
    }
    
    ok
    
  }, error = function(e) {
    stop(paste0("✖ ERROR in ", fitFile, " -> ", e$message), call. = FALSE)
