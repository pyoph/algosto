...
resultsStr <- streaming(Z,params$n/1,1,params$c,params$n,params$d,params$q,params$r)
...
affiche_erreurs_mediane_geometrique(calculErreursM(resultsStr$miter,params$m,params$n),params$n)

#Affichage de l'estimation des erreurs pour l'estimation de Vbarre

affiche_erreurs_mcm(calculErreursV(resultsStr$VIter,resultsSimul$Vvrai,params$n),params$n,params$c)

#Affichage de l'erreur d'estimation des valeurs propres de la MCM

affiche_erreurs_vp_mcm(calculErreursVpMCM(resultsStr$vpMCM,resultsSimul$VpvraiesV,params$n),params$n)

#results$U[99,,]

#Affichage de l'erreur d'estimation des vecteurs propres de la MCM

affiche_erreurs_vectp_mcm(calculErreursVectpMCM(resultsStr$U,eigen(params$Sigma1)$vectors,params$n,params$d),params$n,params$c)
resultsSimul$VcovVrai
#Affichage de l'erreur d'estimation des valeurs propres de la matrice de covariance

affiche_erreurs_cov(calculErreursValPropreCov(resultsStr$lambdaIter,eigen(params$Sigma1)$values,params$n),params$n)


#Comparaison stat khi-deux avec densité théorique Khi deux

densiteHistKhi2(resultsStr$stat,params$d)
#results$outlier_labels
table_contingence(resultsSimul$labelsVrais[1:999],resultsStr$outlier_labels)

#calculErreursSigma(params$Sigma1,results$U,resultsStr$lambdatilde,params$n,params$d)

affiche_erreurs_Sigma(calculErreursSigma(params$Sigma1,resultsStr$U,resultsStr$lambdatilde,params$n,params$d),params$n,params$c)
